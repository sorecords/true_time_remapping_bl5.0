...
#  TRUE TIME REMAPPING
#  Time remapping add-on for Blender 4.4+
#  (c) 2025 Andrey Sokolov (so_records)
...
class TTR_FoSupport(TTR_Helpers):
    ...
    def get_fouts(self):
        if (self.main_sc.use_nodes and self.main_sc.compositing_node_group
        and self.main_sc.compositing_node_group.nodes):
            for nd in self.main_sc.compositing_node_group.nodes:
...
class TTR_RenderSupport(TTR_CommonSupport):
    ...
    def restore_from_tmb(self, context):
        ...
        sc = context.scene
        if not self.main_sc.compositing_node_group or not self.main_sc.compositing_node_group.nodes:
            return
        #------------------------------- Turn off all TMB Mix (Alpha Over) nodes
        for node in self.main_sc.compositing_node_group.nodes:
